...
describe("PGliteStorageProvider", () => {
  ...
  test("insertReasoningChains batch inserts", async () => {
    const embedding1 = Array.from({ length: 384 }, (_, i) => Math.cos(i * 0.1));
    const embedding2 = Array.from({ length: 384 }, (_, i) => Math.sin(i * 0.2));
  ...
  test("searchReasoning respects threshold", async () => {
    ...
    const queryEmbedding = Array.from({ length: 384 }, (_, i) => (i % 2 === 0 ? 1 : -1));
  ...
  test("searchReasoning filters by project", async () => {
    const queryEmbedding = Array.from({ length: 384 }, (_, i) => Math.sin(i * 0.2));
  ...
  test("insertReasoningChain works without sessionId", async () => {
    const embedding = Array.from({ length: 384 }, (_, i) => Math.sin(i * 0.3));
